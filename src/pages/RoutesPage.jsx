...
export default function RoutesPage(props) {
  ...
  React.useEffect(() => {
    let active = true;
    Promise.allSettled([api.listRateLimitPolicies(), api.getAPIKeyOptions(), api.listCORSPolicies()])
            ...
            {routes.map((route) => (
              <tr key={route.id}>
                ...
                <td>
                  ...
                  <small>CORS: {policyName(route.cors_policy_id, corsPolicies, 'none')}</small>
                </td>
